...
> Bundled copy of the standalone `decompose` skill, carried inside `relay` so it needs no
> separate install. The standalone skill (and the research citations behind these defaults, in
> its `references/evidence.md`) lives at `skills/decompose/` in
> https://github.com/msmykowski/claude-skills — optional background, not needed to execute this
> manual.
...
confidence. This manual decides *how to split and sequence* — it does not implement the code.
...
literature, and stacked-PR practice) — cited in the standalone skill's `evidence.md`. Read it
...
trunk. Reach for (the standalone skill's `evidence.md` has the mechanics):
